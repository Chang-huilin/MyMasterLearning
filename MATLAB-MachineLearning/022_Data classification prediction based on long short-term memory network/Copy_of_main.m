...
layers = [
    ...
    lstmLayer(48, 'OutputMode', 'last')       % 第一层LSTM层，输出模式为last
    batchNormalizationLayer                   % 批量归一化层
    reluLayer                                 % Relu激活层
   
    dropoutLayer(0.5)                         % Dropout层，防止过拟合
    fullyConnectedLayer(60)                   % 全连接层，增加神经元数量
    reluLayer                                 % Relu激活层
    
    fullyConnectedLayer(7)                    % 全连接层，输出为7个类别
...
options = trainingOptions('adam', ...         % 使用Adam优化算法
    ...
    'LearnRateDropPeriod', 750, ...           % 经过 750 次训练后 学习率下降
